package com.singlepointsol.ABZ_Final_Project.Vehicle
...
class VehicleActivity : AppCompatActivity() {
                ...
                withContext(Dispatchers.Main) {
                    if (response.isSuccessful) {
                        ...
                        if (!vehicleList.isNullOrEmpty()) {
                            ...
                            // Extract unique Registration Authorities
                            val regAuthorities = vehicleList.map { it.regAuthority }.distinct()
                            ...
                            setupAutoCompleteTextView(
                                binding.regNumberDropdown,
                                regNumbers
                            )
                            setupAutoCompleteTextView(
                                binding.regAuthorityEt,
                                regAuthorities
                            )
                            setupAutoCompleteTextView(
                                binding.owneridEditText,
                                ownerIds
                            )
                        } else {
                            Toast.makeText(
    ...
    /**
     * Helper function to set up an AutoCompleteTextView with given data.
     */
    ...
    private fun saveVehicle() {
        ...
        if (newVehicle == null || newVehicle.regNo.isNullOrBlank()) {
            Toast.makeText(
                this,
                "Please fill in all fields, including Vehicle regNO",
                Toast.LENGTH_SHORT
            ).show()
            return
        }
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                Log.d(
                    "VehicleActivity",
                    "Attempting to add vehicle with regNo: $regNO, Data: $newVehicle"
                )
                ...
                if (response.isSuccessful) {
                    ...
                    Log.e(
                        "VehicleActivity",
                        "Failed to add vehicle: Response Code: ${response.code()}, Error: ${
                    ...
                    runOnUiThread {
                        Toast.makeText(
                            this@VehicleActivity,
                            "Failed to add vehicle: ${response.message()}",
                            Toast.LENGTH_SHORT
                        ).show()
                    }
                }
                ...
                runOnUiThread {
                    Toast.makeText(
                        this@VehicleActivity,
                        "Error adding vehicle: ${e.message}",
                        Toast.LENGTH_SHORT
                    ).show()
                }
            }
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    Log.e(
                        "VehicleActivity",
                        "Failed to update Vehicle: ${response.errorBody()?.string()}"
                    )
                    runOnUiThread {
                        Toast.makeText(
                            this@VehicleActivity,
    ...
    private fun deleteVehicle() {
        val regNO = binding.regNumberDropdown.text.toString()
        if (regNO.isEmpty()) {
            Toast.makeText(this, "Please enter an Vehicle RegNo to delete", Toast.LENGTH_SHORT)
                .show()
            return
        }
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    Log.e(
                        "VehicleActivity",
                        "Failed to delete vehicle: ${response.errorBody()?.string()}"
                    )
                    runOnUiThread {
                        Toast.makeText(
                            this@VehicleActivity,
