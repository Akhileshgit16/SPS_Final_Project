package com.singlepointsol.navigatioindrawerr.Product
...
class ProductActivity : AppCompatActivity() {
    ...
    private fun populateProductIds() {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val response = productService.fetchProductDetails() // Fetch all agents
                withContext(Dispatchers.Main) {
                    if (response.isSuccessful) {
                        val productList = response.body()
                        if (!productList.isNullOrEmpty()) {
                            val agentIds = productList.map { it.productID } // Extract Agent IDs
                            val adapter = ArrayAdapter(
                                this@ProductActivity,
                                android.R.layout.simple_dropdown_item_1line,
                                agentIds
                            )
                            binding.DropdownProductId.setAdapter(adapter) // Set the adapter to the dropdown
                        } else {
                            Toast.makeText(this@ProductActivity, "No Product IDs found!", Toast.LENGTH_SHORT).show()
                        }
                    } else {
                        Toast.makeText(this@ProductActivity, "Failed to fetch Product IDs!", Toast.LENGTH_SHORT).show()
                    }
                }
            } catch (e: Exception) {
                Log.e("ProductActivity", "Error fetching Product IDs: ${e.message}", e)
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@ProductActivity, "Error fetching Product IDs!", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
            ...
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    if (!products.isNullOrEmpty()) {
                        ...
                        binding.insuredIntrestsEditText.setText(firstProduct.insuredInterests)
